...
def parse_all(db_path: str = None, max_pdfs: int = 50) -> dict:
    ...
    for doc in docs:
        ...
        for file_rec in pdf_files:
            ...
            # Detect if text is English (prefer English over Hindi)
            english_score = text.count('the') + text.count('GDP') + text.count('percent')
            hindi_score = text.count('cid:') + text.count('\u0900')
            is_english = english_score > hindi_score
            current_len = len(doc.get("raw_text") or "")
            
            # Update if: empty, or current is Hindi and this is English
            should_update = text and (current_len < 200 or (is_english and hindi_score == 0))
            if should_update:
                conn = get_connection(db_path)
                with conn:
                    conn.execute(
